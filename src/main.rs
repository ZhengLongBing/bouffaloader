...
fn run_cli<W: Write, R: Read, L: OutputPin, SPI, PADS, const I: usize>(
    ...
    enum Base<'a> {
        ...
        /// Reload from sdcard.
        Reload,
        /// Read command to fetch data from a specified address.
        Read{
            addr:& 'a str
        },
        /// Command to write a value to a specified address.
        Write {
            addr: & 'a str,
            val: & 'a str,
        }
                ...
                match command {
                    ...
                    Base::Write {addr, val}=>{
                        match (parse_hex(addr),parse_hex(val)) {
                            ...
                            _ =>cli.writer().write_str("Error: Invalid address or value!").unwrap(),
                        }
                    }
                }
                Ok(())
